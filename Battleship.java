...
public class Battleship{
	...
	int dir = 0;
	int carrierCount = 4;
	printGrid(ugrid);//diag
	while(error) {
	    System.out.print("Awesome! First, we'll place your Carrier. It'll take up  units on the board. Make sure you choose a letter between A and J, and a number between 0 and 9 for the coordinates. enter the x and y coordinates with no space in between, like so: 'D7' :  ");
	...
	ugrid[x][y] = "carrier";
	printGrid(ugrid);//diag
	System.out.print("Do you want the ship to lay horizontally, or vertically? Enter 1 for vertically, or 2 for horizontally: ");
	...
	while (error) {
		...
		if(ugrid[x][y] != null){
			...
			if (dir == 1) {
			    if (x < 5) {
				if (ugrid[x+1][y] != null || ugrid[x+2][y] != null || ugrid[x+3][y] != null || ugrid[x+4][y] != null) {
				    System.out.print("The direction you have chosen will result in an error. Please try again.");
				    battleshipSetup();
			...
			//--------------------------------------------------
			    ...
			    if (y < 5) {
				if (ugrid[x][y+1] != null || ugrid[x][y+2] != null || ugrid[x][y+3] != null || ugrid[x][y+4] != null) {
				    System.out.print("The direction you have chosen will result in an error. Please try again.");
				    battleshipSetup();
    ...
    }//Somehow, we need the while(errortwo) loop to break, but I can't figure out where to break it. It just keeps going back to battleshipSetup().
    /*
    ...
    */
    ...
    public void setRand(String[][] ugrid) {
	...
	printGrid(ugrid);//WHY IS UGRID NULL WHERE ARE THE FREAKING SHIPS GUH.
	//playTurn();
    }
	...
	int ans = 0;
	if (CORD_LETTERS.contains(x)) {
	    ans = CORD_LETTERS.indexOf(x);
	}
	return ans;
    }
